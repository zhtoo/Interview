...
用于动态类型化数据值的容器。 主要用于{@link android.content.res.Resources}用于保存资源值。

TypedValue类在android.util包下。一般在需要计算dp、sp与px之间的关系的时候会被使用。
...
TypedValue在 android.util 包下，说明是一个工具类。 那google提供的View会不会都调用这个方法？

以TextView为例，来探索一下TypedValue：

TextView的构造方法中代码片段， displayMetrics的获取方法:getResources().getDisplayMetrics();
...
PS：context.getResources()是 Context里面的抽象方法 
...
ContextImpl创建是在 ActivityThread 创建时调用getSystemContext方法进行创建的（不同时期创建Context的方法的不同，详细创建方法参照上述的一堆createXXXContext）
...
#### Resources 
    ...
    public class Resources {
		 ...
		 * on orientation, etc).
		 * 返回一个全局共享资源对象，该对象仅提供对系统资源的访问权限（非应用程序资源），并且未针对当前屏幕进行配置（不能使用dimension单位，不会根据方向更改等）。
		 */
		...
		*/
		 ...
		 * AssetManager.
		 * 在AssetManager中现有的assets（资源集合）之上创建新的Resources对象。
		 *
		...
		*
		* @param classLoader 用于加载自定义资源类的包的类加载器，可以是{@code null}来使用系统类加载器
		* @hide
		*/
