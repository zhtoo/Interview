...
题外话：也许是厌烦了一提到时间分发机制就是View一层层传递，true处理，false不处理，吧啦吧啦...又或许是：咦，我明明在onInterceptTouchEvent拦截了事件啊，咋没效果啊？百度一大堆解决方案一个个试。然后就有了这篇没有深度的分析问。
...
如果采用上面的案例，小明一层层问领导，就会得到领导明确的答复。但是这也会出现有些大领导大忙了，小明的请假条一直放在桌上没处理，就搁置了小明的请假，没有给任何回复。接下来就会介绍下责任链模式的优缺点。
...
这里需要结合obtain进行分析，TouchTarget.obtain后，这里有点不容易理解。先记住这步操作时将ViewGroup的子View对应的TouchTarget放到前面
